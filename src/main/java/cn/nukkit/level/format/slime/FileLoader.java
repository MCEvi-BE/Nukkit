...
public class FileLoader {
    ...
    private static final FilenameFilter WORLD_FILE_FILTER = (dir, name) -> name.endsWith(".world");
    ...
    public byte[] loadWorld(String worldName, boolean readOnly) throws Exception {
        if (!worldExists(worldName)) {
            throw new Exception(worldName + " yok");
        }

        RandomAccessFile file = worldFiles.computeIfAbsent(worldName, (world) -> {

            try {
                return new RandomAccessFile(new File(worldDir, worldName + ".world"), "rw");
        ...
        if (!readOnly) {
            ...
            try {
                ...
                throw new Exception(worldName + " overlapping");
            }
        }
    ...
    public boolean worldExists(String worldName) {
        ...
        System.out.println(new File(worldDir, worldName + ".world").toString());
        return new File(worldDir, worldName + ".world").exists();
    }
    ...
    public void saveWorld(String worldName, byte[] serializedWorld, boolean lock) throws IOException {
        ...
        if (tempFile) {
            worldFile = new RandomAccessFile(new File(worldDir, worldName + ".world"), "rw");
        }
    ...
    public void unlockWorld(String worldName) throws Exception {
        if (!worldExists(worldName)) {
            throw new Exception(worldName + " not found");
        }
    ...
    public boolean isWorldLocked(String worldName) throws IOException {
        ...
        if (file == null) {
            file = new RandomAccessFile(new File(worldDir, worldName + ".world"), "rw");
            closeOnFinish = true;
        }
    ...
    public void deleteWorld(String worldName) throws Exception {
        if (!worldExists(worldName)) {
            throw new Exception(worldName + " deletedtefeldfedlef");
        }

        new File(worldDir, worldName + ".world").delete();
    }
}
